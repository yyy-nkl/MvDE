...
def get_gae_feature(adj, features, epochs, l): #它接受四个参数：adj（邻接矩阵），features（节点特征矩阵），epochs（训练周期数），以及 l（一个可能用于控制模型深度或其他参数的变量）
...
def generate_adj_and_feature(network, feature):  #生成图的邻接矩阵和特征矩阵
    features = sp.csr_matrix(feature).tolil().todense()  #将特征矩阵转换为稀疏的CSR（Compressed Sparse Row）格式，然后转换为密集格式。tolil() 方法将稀疏矩阵转换为密集矩阵。

    graph = nx.from_numpy_matrix(network) #使用 NetworkX 库将 NumPy数组格式的邻接矩阵转换为 NetworkX 图对象。
    adj = nx.adjacency_matrix(graph) #从 NetworkX 图对象中提取邻接矩阵
    adj = sp.coo_matrix(adj) #将 NetworkX 的邻接矩阵转换为 SciPy 的 COO（Coordinate）格式的稀疏矩阵
