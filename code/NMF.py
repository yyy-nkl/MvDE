...
def generate_f1(D,train_samples,feature_m,d_data1,feature_MFm, feature_MFd): #D, train_samples, m_features, d_features, NMF_mfeature, NMF_dfeature
    vect_len1 = feature_m.shape[1] #64
    vect_len2 = d_data1.shape[1] #64
    train_n = train_samples.shape[0] #7257
    train_feature = np.zeros([train_n, 2 * vect_len1 + 2 * D]) #(7257, 308)
    ...
    for i in range(train_n):
        ...
        train_feature[i, vect_len1:(vect_len1 + vect_len2)] = d_data1[216*train_samples[i, 0]+train_samples[i, 1], :]
...
def get_low_feature(k,lam, th, A):  #(D, 0.01, pow(10, -4), association
    m, n = A.shape
